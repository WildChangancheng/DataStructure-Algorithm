...
传说中的快排和Bubble Sort一样，也是一种交换排序，通过元素之间的比较和交换位置达到排序的目的。不同的是，冒泡嗲是在每一轮中只把一个元素冒泡排序到数列的一端，比他小的元素移动到数列的另一边，从而吧数列拆解成两个部分。不同的是，冒泡排序是每次移动一个元素到数列的一端，快速排序则是在每一轮挑选一个基准元素，然后把比他大的都放在一边，比他小的都放在另一边，从而把数列拆解成两个部分。这种思路叫分治法。

分治法的核心思路就是把一个大规模问题分解成若干个相同的小规模问题（注意，分解出的子命题和原命题应当是相同问题，只是规模不同而已）。
