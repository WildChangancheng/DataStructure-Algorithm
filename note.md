...
之前我们提过，数据结构是一个二元组，是(D, R)二元组。其中D是数据元素的有限集，R是关系的有限集。类似的，ADT = (D, S, P)，也就是D 是数据对象;  S 是 D 上的关系集;  P 是 D 的基本操作集。（注意，数据结构里的是数据元素，也就是基本元素、ADT里的是数据对象，也就是数据元素的集合）
...
- 时间复杂度：Time Complexity
  ...
  - 定义大O符号(估计最大情况)：`存在两个正的常数 𝒄 和 𝒏𝟎，对于任意 𝒏≥𝒏𝟎，都有𝑻(𝒏 )≤ 𝒄×𝒇(𝒏)，则称 𝑻(𝒏)=𝑶(𝒇(𝒏))`
