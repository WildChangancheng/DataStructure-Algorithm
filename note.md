...
原先该班级由校内二次考核遴选建成，故默认大家拥有较为扎实的基础知识（如高中有OI竞赛基础）。也许部分学弟学妹此前计算机能力相对薄弱，但相信以学弟学妹们的学习能力，有能力克服上手过程中的部分台阶。
...
树是一种重要的非线性结构，不会产生循环定义。从数学上，我们认为树是特殊的图，是n个结点的有限集。在任意一棵非空树中

- 根：有且仅有唯一的根结点root
- 子树：n>1时，除去根外剩下的n-1个结点可以分成若干个`互不相交`的有限集，各自也成树，称之为`子树`，subtree（英文中经常表示接下来和继承义的prefix就是sub）

> 可以注意到，树是递归概念。也就是定义树的过程中，又用到了树的概念
...
- 树是一种分枝结构，树中只有根结点没有前驱，其余结点有零个或多个后继
  - 注意！一个结点的前驱不一定是他的parent。这和遍历方式（前序、中序什么的）有关系
- 除根以外，其他节点都`有且仅有`一个前驱（看具体的遍历方式，不同遍历方式可能得到不同的前驱）。并且`存在唯一`通往root的路径
...
- 结点层：根结点的层定义为1，其它依此类推（从1开始！）；
- 树的深度：树中最大的结点层；
- 结点的度：结点子树的个数（有多少个孩子节点）；
- 树的度： 树中最大的结点度（取结点度的max）；
- 叶子结点：也叫终端结点，是度为0的结点（不是分枝节点的结点）
- 分枝结点：度不为0的结点（不是叶子节点的节点）；
- 有序树：子树有序的树，如：家族树；
- 无序树：不考虑子树的顺序；
- 森林：互不相交的树集合（可以理解为很多树的集合）

容易发现，去掉一个树的根后子树成森林；把森林的所有树连在一个作为根的结点上得到一个树。

### 5.2 二叉树

按当前定义，树的形状极其自由。我们在这里限定一个结点的度必小于等于2且树是有序树。这样的树称为二叉树。也就是说，二叉树有`有序`、`节点度小于等于2`和`递归定义`的特点。

我们称“排满n层”（也就是倒数第二层的每个结点都有2个叶子结点）的二叉树为满二叉树；所有节点序号和满二叉树相同的二叉树为完全二叉树（也就是在按照完全二叉树的顺序排列结点，只是不一定排满了）。

容易证明，二叉树具有性质：

- 二叉树的第i层最多有$2^{i-1}$个结点
- 深度为n的二叉树最多有$2^{n}-1$个结点（也就是说，第n+1行的第一个结点是第$2^n$个结点）
  - 这个性质来自于$1+2+4+...+2^n = \frac{2^n-1}{2-1}=2^n-1$
- 对任意二叉树，记度为0的结点有$n_0$个，度为1的结点有$n_1$个，度为2的结点有$n_2$个（也就是记度为i的节点有$n_i$个）。存在关系$n_0 = n_2 + 1$（叶子结点的个数可以直接用“儿女双全结点”+1。这个儿女双全结点是我学疯了自己发明的口诀，别去别的地方用）
- 具有n个结点的完全二叉树深度为$[\log_2{n}]+1$（也就是看有几个2，最后没凑够一层的+1）
- 若对含有n个结点的完全二叉树从上到下且从左到右进行1~n的编号（就是上面说的那种对满二叉树编号的编号方式）
  - i=1的是根节点
  - $2i > n$, 则该结点无左孩子，否则编号为$2i$的结点就是他的左孩子
  - $2i + 1 > n$, 则该结点无右孩子，否则编号为$2i+1$的结点就是他的右孩子
  - 通过这条性质就可以利用编号推出左孩子/右孩子的编号。也就是$2i$和$2i+1$
  - 所以这条性质非常重要。因为按着这种思路，可以把编号作为秩，从而实现用线性结构存储二叉树。

### 5.3 用线性结构存储二叉树

就像上面说的，当二叉树是满二叉树或者一般的完全二叉树时，都可以通过最后一条性质计算编号作为秩，，从而实现用线性结构存储二叉树。

那一般的二叉树呢？一般的二叉树和满二叉树的区别显然在于中间缺少结点，导致编号没有这么完美的顺序。那我们不妨用0填充中间没有填满的结点（虚设结点法）。这样一来也可以当做满二叉树处理。这种方案的缺点在于，如果树非常畸形，那就填充了太多0，占用空间太大。所以这个方法适合尽量满的二叉树。遇见空一些的二叉树时，空间利用率可能会很低下。

### 5.4 用链式结构存储二叉树

二叉树的链式存储方案之一是二叉链表。每个结点包括三个域：数据域、左指针域和右指针域。

- n个节点的二叉树中，共用`n+1`个空指针域。
  - 有个巧妙的办法，就是n个节点共有2n个指针域，其中root不占用指针域，剩下n-1个结点每个占用一个指针域，那么剩下`2n-(n-1)=n+1`就是空着的指针域
- 本笔记项目中还有一个BasicAlgorithm文件夹，里面的二叉树部分就是用面向对象语言的`引用`替代C语言的指针实现了二叉链表
- 二叉链表的缺陷在于容易自上而下找孩子，却不容易向上找到双亲节点。

为了解决难以溯源的问题，我们可以引入三叉链表。三叉链表在二叉链表的基础上加上了双亲指针，也就是说具有`lch, data, rch, parent`四个域，其中lch、rch和parent都是指针域。用C语言描述，就是：
...
遍历，即按某种搜索路径访问二叉树的每个结点，而且每个结点仅被访问一次。访问，是指对结点进行各种操作的简称，包括输出、查找、修改等等。

遍历当然是任何结构都具有的特征。就像之前学习的线性结构，只有一条搜索路径 (因为每个结点均只有一个后继)，顺着顺序就可以完成遍历。但是现在学习的非线性结构的遍历没有那么简单的后继关系，遍历也就不再是显而易见的事了。

换言之，我们其实在寻找一种搜索路径，能不重复、不遗漏地访问所有元素。

约定先左后右，有3种遍历方法：T L R、L T R、L R T，分别称为
...
> 可以感受到，遍历二叉树的过程实质是把二叉树的结点进行线性排列的过程
>
> 也就是说，遍历的实质是把二叉树转化成线性序列
...
我们已经可以通过二叉链表和三叉链表查找一个节点的双亲节点和孩子节点，但是怎么查询一个节点的后继和前驱节点呢？又不行了。

我们用类似的思路，多加一点指针上去。这样得到的就是线索二叉树。线索就是指向其前驱节点和后继节点的指针。多加的指针存在哪呢？

我记得左右孩子节点的位置有n+1个空位，能不能放在那里？如果我们加以证明，会发现完全可以把前驱和后继放在原本留给左孩子右孩子指针的地方。我们先假设可以这么做，具体的证明留在后面。

那这样的话，我怎么知道lch指向的是双亲节点还是左孩子？总不能蒙一个，管爹叫儿子管儿子叫爹吧。所以我们需要为结点添加两个新的
