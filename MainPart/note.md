...
这里我们需要重新强调一部分最优子结构的性质。最优子结构性质是说，我们的最终决策在特定的拆分组合下，得到子问题1和2，一定达到子问题1和子问题2的最优解，也就是强调“存在”；但不是“任意”。最优解并不是在任意拆分组合下都是最优解；也不意味着任意拆分组合下的最优解合起来也是最优解。

比如我们有100元，A物品5元，B物品2元，怎么样能尽可能不浪费钱（也就是把这些钱全花完）？很容易想到，不管我们是买20个A还是50个B，总有办法能把100元全花完。换句话来说，在最优解下，这100块钱一定是一分不剩的。 但如果我们随意地把100元拆分乘51和49。很显然，不论我们怎么做，51元应该是可以花完的（9个A花费45元，3个B花费6元）；49元总是要剩下一些，最理想的情况下应该是花48元，剩下1元。也就是说：组合起来，我们能花光100元，但是在51+49这个组合下，我们是花不完的，最少也会浪费一块钱。

但是我们如果把100元拆成40和60元的组合，很容易发现，最优解能花光60块钱也能花光40块钱，把他们的最优解合起来一定就是100元。所以最优解强调的是在特定的组合里，最优解可以由子问题的最优解组合得到，并没有说任何组合都可以推出最优解，这是不成立的。

### 22.7 0-1背包问题

背包问题是一个非常好的动态规划入门题目，也是动态规划中最常见的文体形式。其中最常见的问题之一就是0-1背包问题。

> 给定n个物品，第i个物品的重量为wgt[i-1]，价格为val[i-1]，和一个容量为cap的背包，每个物品只能选择一次，怎么在限定背包的容量（也就是确定cap）的情况下使得放下的物品最大。

出现了很多“最“，很显然是一个优化问题。我们思考一下，发现确实符合最优子结构问题，也符合无后效性，也具有重叠子问题特性。所以这道题可以尝试用动态规划的方式解决。
